    ...
    */
    class SRGPDF 
    {
        ...
        public function createHeader()
        {
            ...
            <button class="no-print" onclick="printReport()">Imprimir ou Salvar PDF</button>
            <div id="printable">
            ...
            if($this->category != "0") {
                $this->html .= 'Tipo de Atividade: '.$this->formatCategory($this->category).'<br>';
            } else {
                $this->html .= 'Tipo de Atividade: Todos<br>';
            }
            ...
            //Continue at render or render no one
        }
        ...
        public function loadActivities($u_id = null) 
        {
            ...
            switch ($this->type) {
                case 'by_center':
                    ...
                    $num_atvs = count($activities);
                    $this->html .="Atividades retornadas: ".$num_atvs;
                    $this->html .= '</td>
                        </tr>
                    </table><br>';
                    $this->html .= '<hr><b>'.$center->name.'</b><hr>';
        ...
        private function getActivitiesByCoursesIds($courses_id)
        {
            ...
            if(count($courses_id) > 0) {
                ...
                if(count($activities_ids)>0) {
                    ...
                    $sql .= " and status = ?";
                    $sql .= " order by year desc, unity_id asc, category, title";
                    $activities_ids[] = "Y";
        ...
        private function renderActivities($activities)
        {
            foreach ($activities as $activity) {
                $this->html .= '<table width="100%" border="1">
                        <tr><td><b>Ano de execução: '.$activity['year'].' <br> Unidade: '.$activity['unity_name'].'<br>
                            Tipo de ativade: '.$this->formatCategory($activity['category']).'
                        <b></b></td></tr>
                        <tr>
                            <td>
                                <u>Atividade</u>: '.$activity['title'].'<br>
        ...
        {
            $this->html .= '</div><script type="text/javascript" src="../wp-content/plugins/srgccsa/public/js/jspdf/dist/jspdf.min.js"></script>';
            return $this->html;
        }
    }
?>